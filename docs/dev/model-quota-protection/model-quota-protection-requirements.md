...
|----|--------|----------|--------|----------|------|
| F-001 | 动态阈值过滤 | 1. 设置阈值为 5% 2. 构造账号 A (剩余 3%) 3. 发起请求 4. 预期：账号 A 被跳过 | P0 | Done | ☑ |
| F-002 | 自动恢复映射 | 1. 账号 A 剩余 0.5% (已禁用) 2. 更新账号 A 配额为 10% 3. 发起请求 4. 预期：账号 A 重新被选中 | P0 | Done | ☑ |
| F-003 | 警告日志输出 | 1. 触发配额跳过 2. 检查日志 3. 预期：出现 "Account X skipped due to low quota (0.8% < 1.0%)" | P1 | Done | ☑ |
