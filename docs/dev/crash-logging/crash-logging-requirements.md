...
|----|--------|----------|--------|----------|------|
| F-001 | Rust panic 自动记录 | 1. 触发 Rust panic 2. 检查日志文件存在且包含堆栈 | P0 | 待分配 | ☐ |
| F-002 | JS 全局错误捕获 | 1. 触发 JS 错误 2. 验证日志包含错误消息和堆栈 | P0 | 待分配 | ☐ |
| F-003 | React 组件错误捕获 | 1. 组件渲染错误 2. Error Boundary 显示 UI 3. 日志已记录 | P0 | 待分配 | ☐ |
| F-004 | 系统信息附加 | 1. 触发崩溃 2. 日志包含 OS/版本/内存信息 | P1 | 待分配 | ☐ |
| F-005 | 日志按时间轮转 | 1. 创建 8 天前的日志文件 2. 重启应用 3. 旧日志被删除 | P0 | 待分配 | ☐ |
| F-006 | 日志按大小轮转 | 1. 写入超过 10MB 数据 2. 检查是否创建新文件 | P0 | 待分配 | ☐ |
| F-007 | 日志总大小限制 | 1. 生成 110MB 日志 2. 检查总大小被控制在 100MB 内 | P1 | 待分配 | ☐ |
| F-008 | 设置页"打开日志" | 1. 进入设置页 2. 点击按钮 3. 系统文件管理器打开日志目录 | P1 | 待分配 | ☐ |
| F-009 | 边缘:日志目录无权限 | 1. 模拟无写权限 2. 系统降级到临时目录 | P2 | 待分配 | ☐ |
| F-010 | 边缘:日志目录不存在 | 1. 删除日志目录 2. 触发日志写入 3. 自动创建目录 | P2 | 待分配 | ☐ |
| F-011 | 边缘:sysinfo 获取失败 | 1. 模拟 sysinfo 错误 2. 日志仍写入(仅缺系统信息) | P2 | 待分配 | ☐ |
